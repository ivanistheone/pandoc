...
{- |
   ...
   Maintainer  : Ivan Savov <ivan.savov@gmial.com>
   Stability   : alpha
...
blockToMediaWiki opts (Header level inlines) = do
  contents <- inlineListToMediaWiki opts inlines
  let eqs = replicate (6-level) '='             -- dokuwiki H1 is 6*=, H2 5*=, H3 4*= etc...
  return $ "\n" ++ eqs ++ " " ++ contents ++ " " ++ eqs ++ "\n"
...
inlineToMediaWiki _ (Math t str) = return $ if t == InlineMath
                                                then "$" ++ str ++ "$"
                                                else "\n\\[\n   " ++ str ++ "\\]\n"
                                 -- note:  str should NOT be escaped
                                 -- added the handler displaymath stuff.. 
